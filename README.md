...
---
通过调用 Bitwarden CLI 管理 SSH 登录，比如 `bwssh s01` 直接从 Bitwarden 中取出主机地址、端口、用户名、**SSH 私钥或密码** 登陆自己的服务器，**无需本地 Desktop App，无需维护 `~/.ssh/config`**。 全程在 term 中操作。
...
- **内存与会话安全**：凭据与临时 Key / AskPass 脚本均严格隔离，连接结束即刻自动销毁清理。
